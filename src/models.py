...
def _require_dgl():
	try:
		...
		from dgl.nn import GATConv  # noqa: F401
		return dgl, GATConv
	except Exception as e:
		raise ImportError(
			"DGL is required for GDN and MTAD_GAT models. "
			"Install a compatible 'dgl' build or choose a different model."
		) from e
...
# Choose a safe number of attention heads for a given model dim
# to avoid extremely tiny head dimensions which can be unstable on some CUDA setups.
def _stable_nhead(d_model: int, max_heads: int = 8) -> int:
	max_h = min(max_heads, d_model)
...
class MTAD_GAT(nn.Module):
	def __init__(self, feats):
		...
		# Lazy import DGL only when this model is instantiated
...
class TranAD_Basic(nn.Module):
	def __init__(self, feats):
		...
		nhead = _stable_nhead(feats)
		encoder_layers = TransformerEncoderLayer(d_model=feats, nhead=nhead, dim_feedforward=16, dropout=0.1)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers = TransformerDecoderLayer(d_model=feats, nhead=nhead, dim_feedforward=16, dropout=0.1)
...
class TranAD_Adversarial(nn.Module):
	def __init__(self, feats):
		...
		d_model = 2 * feats
		nhead = _stable_nhead(d_model)
		encoder_layers = TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=16, dropout=0.1)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers = TransformerDecoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder = TransformerDecoder(decoder_layers, 1)
		self.fcn = nn.Sequential(nn.Linear(2 * feats, feats), nn.Sigmoid())
...
class TranAD_SelfConditioning(nn.Module):
	def __init__(self, feats):
		...
		encoder_layers = TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=16, dropout=0.1)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers1 = TransformerDecoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = TransformerDecoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=16, dropout=0.1)
