...
if __name__ == '__main__':
	...
	if not args.test:
		...
		# Export ROC/AUC plots to plots/<model>_<dataset>/
		roc_base = f"plots/{args.model}_{args.dataset}"
		plot_roc(loss, labels, out_file=f"{roc_base}/roc_micro.png", title=f"{args.model} on {args.dataset} ROC (micro)")
		plot_roc_per_dim(loss, labels, out_dir=f"{roc_base}/roc_per_dim")
	...
	for i in range(loss.shape[1]):
		...
		# pandas>=2.0: initialize DataFrame on first insert, then append rows
